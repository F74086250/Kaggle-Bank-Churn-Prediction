...
def test(Var_name = "show_all",Srand_s = 29,Srand_e = 29,Neighbor_s = 3,Neighbor_e = 3):
    ...
    if optimal:
        ...
        Neighbor_s,Neighbor_e = Optimal_Var2,Optimal_Var2
    for i in range(Srand_s,Srand_e+1):
        for j in range(Neighbor_s,Neighbor_e+1):
            ...
            if self_testing:
                if (acc*0.3+prec*0.4+f1score*0.3) > finalscore:
                    ...
                    print("Current final(max) = ",finalscore,"\nSrand(record) = ",Var1,"\nNeighbor(record) = ",Var2)
                    print()

                Total = (Srand_e-Srand_s+1)*(Neighbor_e-Neighbor_s+1)
                progress =  (i-Srand_s)*(Neighbor_e-Neighbor_s+1)+(j-Neighbor_s)
...
def self_optimizing(v1=25,v2=60,v3=3,v4=20,Apply = True):
